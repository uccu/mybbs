(function(){
    ...
    var pluginUrl = location.origin+'/'+folder[1]+'/';
    j(function(){
    ...
    var breadcrumb = function(a){
        ...
        if(a && folder[3]){
            ...
            if(folder[4] && !folder[4].match(/\d+/i))
                e += '<li><a href="'+pluginUrl+folder[2]+'/'+folder[3]+'/'+folder[4]+(folder[5]?'/'+folder[5]:'')+(folder[6]?'/'+folder[6]:'')+'">详情</a></li>';
        }
        e += '</ol>';
        ...
        for(var i=0;i < arguments.length;i++){
            if(typeof arguments[i] === 'object'){
                for(var d in arguments[i])
                    e += '<a href="'+pluginUrl+folder[2]+'/'+d+'" class="list-group-item">'+arguments[i][d]+'</a>'
            }else continue;
        }
        e += '</div></div>';f.before(e);
        if(folder[3]){
            breadcrumb(j('.subnav .list-group-item[href="'+pluginUrl+folder[2]+'/'+folder[3]+'"]').addClass('cd active').removeAttr('href').html());
        }else for(var i=0;i < arguments.length;i++){
            if(typeof arguments[i] === 'string'){
                breadcrumb(j('.subnav .list-group-item[href="'+pluginUrl+folder[2]+'/'+arguments[i]+'"]').addClass('cd active').removeAttr('href'));break;
            }
        }
