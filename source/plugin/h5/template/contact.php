<!--{subtemplate header}-->
<style>
h4{font-size:20px;margin-bottom:20px}
p{color: #999;
    line-height: 24px;}
</style>
<div class="container" style="margin-top: 20px;">
    <h4>关于运维卫士</h4>
    <p style="margin-bottom: 0">
        运维卫士APP隶属于上海擎测机电工程技术有限公司版权所有，APP主要为电厂及机电相关的用户提供综合性的在线咨询与问诊，以及针对性的课程与考试的专业服务平台，为用户提供最全面的设备问题解决方案。
    </p>

</div>


<div class="container" style="margin-top: 40px;">
    <h4>联系方式</h4>

    <div class="row" style="margin-bottom:15px;color: #999;">
        <div class="col-xs-2 tr">
            <img src="/pic/h5/contact/phone@2x.png" alt="" style="height: 49px;">
        </div>
        <div class="col-xs-10" style="background:url(/pic/h5/contact/b.png);background-size: 100% 100%;padding: 10px 0 10px 30px;width: 200px;">
            <font style="color:orange;font-size:16px">400-666-666</font> （免费）
        </div>
    </div>
    <div class="row">
        <div class="col-xs-2 tr">
            <img src="/pic/h5/contact/email@2x.png" alt="" style="height: 49px;">
        </div>
        <div class="col-xs-10" style="background:url(/pic/h5/contact/b.png);background-size: 100% 100%;padding: 10px 0 10px 30px;width: 180px;color: #999;">
            yunweiws@126.com
        </div>
    </div>
    <div class="row">
        <div class="col-xs-2 tr">

        </div>
        <div class="col-xs-10">
             <p style="color:#015caf;margin-top:25px">欢迎前来我司参观与考察</p>
        </div>
    </div>
   
    <div class="row" style="margin-bottom:15px;color: #999;">
        <div class="col-xs-2 tr">
            <img src="/pic/h5/contact/addr@2x.png" alt="" style="height: 49px;">
        </div>
        <div class="col-xs-10" style="background:url(/pic/h5/contact/b.png);background-size: 100% 100%;padding: 10px 0 10px 30px;width: 240px;font-size: 10px;">
            上海市长宁区延安西路777号601室
        </div>
    </div>
    
</div>


<div class="pf bbg" style="width:100%;bottom:0;left:0;z-index:1">
    <img src="/pic/h5/d.png" alt="" class="img-responsive">
    <span class="db pa clos" style="height:100%;width:40px;z-index:3;top:0;left:0"></span>
    <script>j('.clos').click(function(){j('.bbg').fadeOut()})</script>
</div>

<!--{subtemplate tool:footer}-->